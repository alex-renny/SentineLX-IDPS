...
router.get("/", requireAdmin, async (_req, res) => { try { res.json({ success: true, settings: await getSettings() }); } catch (error) { res.status(500).json({ success: false, message: "Unable to load settings" }); } });
router.put("/", requireAdmin, async (req, res) => {
  try {
    ...
    audit("SETTINGS_CHANGED", { username: req.user.username, preventionMode: settings.prevention.mode });
