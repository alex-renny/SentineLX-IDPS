...
export default function Login() {
  ...
  const login = async (event) => { event.preventDefault(); setError(""); try { setBusy(true); const response = await api.post("/auth/login", { username, password }); sessionStorage.setItem("sentinelx_token", response.data.token); connectSocket(); navigate("/"); } catch (requestError) { setError(requestError.response?.data?.message || "Unable to sign in"); } finally { setBusy(false); } };
